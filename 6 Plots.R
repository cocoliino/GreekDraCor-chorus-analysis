...
fig1 <- ggplot(chorus_plays, aes(x = firstAuthorName, y = chorus_degree, fill = firstAuthorName)) +
  geom_boxplot(alpha = 0.85, color = "black") +
  geom_jitter(width = 0.2, alpha = 0.4, size = 1.5) +
  scale_author_fill +
  labs(
    title = "Chorus Degree Centrality",
    x = NULL,
    y = "Degree"
  ) +
  theme_pub +
...
# Figure 10: Chorus Genre Distribution -----
# stacked bar chart of chorus_type by normalizedGenre, filled by chorus_type 
# this shows the distribution of chorus types (group vs individual vs mixed) across different genres
# we can compare which genres tend to use more group choruses vs individual choruses
# the stacked bars show the proportion of each chorus type within each genre,
# allowing us to see if certain genres favor specific chorus structures
fig10 <- ggplot(chorus_plays, aes(x = normalizedGenre, fill = chorus_type)) +
  geom_bar(position = "fill", color = "black", alpha = 0.9) +
  scale_chorus_fill + 
  labs(
    title = "Distribution of Chorus Structures by Genre",
    x = NULL,
    y = "Proportion",
    fill = "Chorus Type"
  ) + 
...
ggsave("fig10_genre_chorus_distribution.png", fig10, width = 6, height = 4, dpi = 300)
